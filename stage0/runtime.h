...
struct KaiValue {
    /* Koka-packed header (kk_header_t shape): 8 bytes. Mirrors stage2's
     * packed header so the variant node is 48 B (8 header + 5×8 slots),
     * down from 80. `variant_tag` (uint16) and slot count (uint8) live in
     * the header; `slot_mask` / `variant_name` moved to tag→* tables. */
    int32_t  rc;
    uint8_t  tag;
    uint8_t  var_n_args;
    ...
    union {
        ...
        /* Variant slots live INLINE overlapping the union (a variant uses
         * none of the named members). `variant_tag` / slot count are in the
         * header; `slot_mask` / `variant_name` in tag→* tables. Node = 8 B
         * header + n*8 slots = 48 B for 5 slots. The slots are NOT a union
         * member: a flexible array member in a union — or in an otherwise
         * empty wrapper struct — is rejected by C99 (Linux clang errors;
         * Apple clang silently accepts the GNU extension). Access them via
         * the `kai_var_slots(v)` macro below, which casts `&v->as` to
         * `KaiVarSlot *` — identical address (union offset), zero size cost. */
        struct {
            KaiFn       fn;
            int32_t     arity;
            int32_t     n_captures;
        ...
        /* FFI v2 (#417): opaque C handle — borrowed external pointer,
         * never freed by RC (the driver owns the C resource). */
        void *foreign_ptr;
    } as;
};

/* Variant slots for a KAI_VARIANT node. The slots overlap the union
 * `as` at offset 0 (a variant uses none of the union's named members),
 * so `&v->as` cast to `KaiVarSlot *` IS the slot array. This replaces
 * the old `v->as.var.var_slots[i]` flexible-array member, which was a
 * GNU extension (FAM in a union / in an otherwise-empty wrapper struct)
 * that Linux clang rejects under -std=c99. Address and node layout are
...
static inline int32_t kai_head_tag(KaiValue *v) {
    ...
    switch ((KaiTag) v->tag) {
        ...
        case KAI_FOREIGN: return KAI_HEAD_ANON;  /* opaque handle (#417) */
...
/* Koka-packed header: per-node variant_name / slot_mask are gone; tag→*
 * tables recover them (populated at first construction). Mirrors stage2. */
...
static const char *kai_variant_name_of(int32_t tag) {
    if (tag >= 0 && tag < kai_varname_table_cap && kai_varname_table[tag] != NULL) return kai_varname_table[tag];
    return "";
}
static uint32_t *kai_slotmask_table = NULL;
static int kai_slotmask_table_cap = 0;
static uint8_t *kai_slotmask_seen = NULL;
 ...
 *
 * The counter is monotonic high-water (commit, not live). stage0 has no
 * slab allocator, so it is charged per object at each grow point — cell
 * calloc, variant block, slot array, arena chunk, and string/array
 * payloads — so no allocation path grows past the ceiling uncounted.
 * Single OS thread (fibers share it), so a plain global needs no sync. */
...
static KaiVarSlot *kai_slots_alloc(int n) {
    return (KaiVarSlot *) kai_heap_malloc((size_t) n * sizeof(KaiVarSlot));
}
...
/* Variant block: header + n inline slots in one allocation (Koka-packed,
 * 48 B for 5 slots). Slots overlap the union at `offsetof(KaiValue, as)`,
 * so the block size is that offset + n slots. Mirrors stage2's allocator.
 * Using `offsetof` (rather than the old `&((KaiValue*)1)->...` bogus-pointer
 * trick) is both portable and free of the UBSan misaligned-pointer warning. */
static size_t kai_var_block_size(int n) {
...
static KaiValue *kai_alloc_var(int n) {
    kai_heap_charge(kai_var_block_size(n));
    KaiValue *v = (KaiValue *) calloc(1, kai_var_block_size(n));
    if (!v) { fprintf(stderr, "kai: out of memory\n"); exit(1); }
    v->rc = 1;
    ...
    kai_rc_live_now++;
    if (kai_rc_live_now > kai_rc_live_peak) kai_rc_live_peak = kai_rc_live_now;
    if ((int) KAI_VARIANT >= 0 && (int) KAI_VARIANT < 16) kai_rc_alloc_by_tag[(int) KAI_VARIANT]++;
    return v;
}
...
static KaiValue *kai_incref(KaiValue *v) {
    ...
    KAI_PROF_ENTER();
    v->rc++;
    /* #812 — counter is ALWAYS compiled (parallels kai_rc_alloc_total),
     * reported only under the KAI_TRACE_RC env var at exit. Keeping it
     * behind `#ifdef KAI_TRACE_RC` made every `kai build`-produced binary
     * report incref_total=0 (the wrapper does not pass -DKAI_TRACE_RC),
     * so every "RC balanced" gate that read this line passed vacuously. */
    kai_rc_incref_total++;
#ifdef KAI_TRACE_RC
...
/* Issue #817 — recycle-this-cell tail shared by kai_free_value's
 * non-cons cases and kai_free_cons_spine's per-cell reclaim. (stage0
 * has no FAM variant block pool, so the recycle is the uniform
 * cell-pool-or-free.) Reads `_kc->tag` before the poison stamp. */
...
/* Issue #817 — iterative unique-cons-spine free; O(1) stack for a long
 * list (the recursive `kai_decref(tail)` overflowed a 64 KiB fiber stack
 * once filter/map stopped leaking the spine). Precondition: `v` is a
 * KAI_CONS cell whose rc just hit 0. Heads and shared/non-cons tails go
 * through `kai_decref` (counters + cascade intact); the unique spine is
 * consumed by the loop, each cell reclaimed via KAI_RECYCLE_CELL. */
static void kai_free_cons_spine(KaiValue *v) {
    for (;;) {
        ...
        kai_decref(head);
        KAI_RECYCLE_CELL(v);
        /* stage0 does not tag immediates, so no kai_is_value guard; a
         * null / saturated-singleton / non-cons / shared tail hands off
         * to kai_decref. */
        if (!tail ||
            tail->rc == INT32_MAX ||
 ...
 * rc 0, its name-free already recorded, and its slots below `next_slot`
 * already released. A unique variant child transfers ownership to the
 * walk (rc set to 0, no decref — that per-node decrement is the traffic
 * this path deletes) through a worklist bounded by tree depth; shared,
 ...
 * guard) must not tax the common shared-children free. Slots are inline
 * (overlap the union) — each block frees whole via KAI_RECYCLE_CELL,
...
static void kai_free_value(KaiValue *v) {
    ...
    switch ((KaiTag) v->tag) {
        case KAI_STR:
            ...
            break;
        case KAI_CONS:
            /* Issue #817 — iterative spine free (reclaims v + the unique
             * tail spine, then returns; must NOT fall to the post-switch
             * tail, which would double-free v). */
        ...
        case KAI_VARIANT: {
             ...
             * all-PTR). A unique variant child hands the whole subtree
             * to the iterative walk, which recycles v and returns —
             * must NOT fall to the post-switch tail (that would
             * double-free v). Shared children take kai_decref's rc>1
             * arm unfolded: one rc load, no re-checks. Slots are INLINE
             * (overlap the union) — freed with the block. */
            KAI_VAR_NAME_FREE(kai_variant_name_of(v->variant_tag));
...
static KAI_RC_NOINLINE KaiValue *kai_variant_u(int32_t tag, const char *name,
    ...
    /* Register tag→name and tag→mask up front so ALL paths (nullary,
     * immortal, cold) populate the tables — stage0's match uses
     * strcmp(kai_variant_name_of(tag), ...) and a missing name makes
     * every arm miss → "non-exhaustive match". */
...
static KaiValue *kai_arena_variant(int32_t tag, const char *name, int n,
    ...
    /* Bump the whole variant block (header + n inline slots) so the slots
     * live in the `var_slots` FAM, same layout as the heap allocator. */
...
static KaiValue *kai_op_div(KaiValue *a, KaiValue *b) {
    ...
    if (a->tag == KAI_INT && b->tag == KAI_INT) {
        if (b->as.i == 0) { kai_trap_abort("divide by zero"); }
...
static KaiValue *kai_op_mod(KaiValue *a, KaiValue *b) {
    KaiValue *r;
    if (a->tag == KAI_INT && b->tag == KAI_INT) {
        if (b->as.i == 0) { kai_trap_abort("mod by zero"); }
        r = kai_int(a->as.i % b->as.i);
    } else { fprintf(stderr, "kai: type mismatch in %%\n"); exit(1); }
    kai_decref(a); kai_decref(b);
    return r;
 ...
 * (POSIX keeps the low 8 bits); any other return type is 0. Returning the
 * code lets libc run the full exit path, so buffered stdio is still
 * flushed. Mirrors stage2's kai_main_exit_status. */
static int kai_main_exit_status(KaiValue *result) {
    return (result != NULL && result->tag == KAI_INT)
         ? (int) (result->as.i & 0xff) : 0;
}
...
static KaiValue *kai_core_mailbox_recv_timeout(KaiValue *pid, KaiValue *timeout_nanos) {
    ...
    int64_t ns = kai_take_int(timeout_nanos);
...
static KaiValue *kai_default_nettcp_recv_timeout(void *self, KaiValue *c,
    ...
    if (cap > (1 << 20)) cap = 1 << 20;  /* 1 MiB ceiling for v1 */
    int64_t budget = (ns && ns->tag == KAI_INT) ? ns->as.i : 0;
 ...
 * v1 — datagram sockets do not park the fiber the way the NetTcp R2
 * reactor (issue #630) does; the readiness reactor lifts both at
 * once in a follow-up (m8.x). Maps directly to POSIX `SOCK_DGRAM`
 * sockets via libc, the same FFI-to-libc pattern as NetTcp above.
 * Errors return `Err(strerror(errno))`; success wraps the handle /
 * payload in records matching the surface types the compiler
 * declares (stage2/compiler/driver.kai):
 *
 *   UdpSocket  = { fd: Int, port: Int }
 *   SocketAddr = { host: String, port: Int }
 *
 * The `port` slot on UdpSocket carries the kernel-assigned port back
 * to the caller (the `bind(port=0)` use case) via getsockname, the
 * same trick `kai_default_nettcp_listen` uses for Listener.
 *
 * v1 keeps to AF_INET (IPv4). `[Byte]` lands as `[Int]` (no
 * first-class Byte), each element truncated to a byte before
 * sendto. recv reformats the source address back into a SocketAddr
 * record via inet_ntop / ntohs. SocketAddr.host is a textual IPv4
 * dotted-quad — same v1 simplification documented for the TLS lane.
 */

/* Build a UdpSocket record `{ fd, port }`. Field names are the
 * runtime contract — the kaikai-side `type UdpSocket = { fd: Int,
 * port: Int }` (builtin_udpsocket_decl) and the _kai_net_record_fd
 * reader below match on these strings. */
...
/* Build a SocketAddr record `{ host, port }`. Mirrors the kaikai
 * `type SocketAddr = { host: String, port: Int }` declaration. The
 * `host` slot is a textual IPv4 address; `recv` fills it from the
 * datagram source via inet_ntop. */
...
/* Pull the textual `host` slot out of a SocketAddr record into
 * `out` (NUL-terminated, truncated to `cap-1`). Returns 0 on
 * success, -1 if the value is the wrong shape. */
...
/* Pull the `port` slot out of a SocketAddr / UdpSocket record.
 * Returns -1 if the value is the wrong shape. */
...
/* bind(host, port) -> Result[UdpSocket, String]. host = "" or
 * "0.0.0.0" binds INADDR_ANY; a specific IPv4 dotted-quad also
 * works. port = 0 asks the kernel for an ephemeral port; we read it
 * back via getsockname so callers don't need a separate op. The
 * socket is left blocking in v1 (no reactor wiring). */
static KaiValue *kai_default_netudp_bind(void *self, KaiValue *host, KaiValue *port, KaiCont *k) {
    ...
    /* SO_REUSEADDR mirrors the NetTcp listen convention — a probe
     * that crashed seconds ago can rebind without waiting out the
     * kernel's hold on the port. */
    int yes = 1;
    setsockopt(fd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes));
    ...
    if (host_buf[0] == '\0') {
        ...
        /* Fall back to getaddrinfo for hostnames (e.g. "localhost"). */
        struct addrinfo hints;
        memset(&hints, 0, sizeof(hints));
        hints.ai_family   = AF_INET;
...
/* send(s, dst, data) -> Result[Int, String]. data is a [Int] each
 * element truncated to a byte. dst is a SocketAddr `{ host, port }`;
 * host is parsed as an IPv4 dotted-quad via inet_pton. Returns the
 * byte count sent. A single sendto either delivers the whole
 * datagram or fails — no partial-write loop (unlike the TCP stream
 * send). */
static KaiValue *kai_default_netudp_send(void *self, KaiValue *sock, KaiValue *dst, KaiValue *data, KaiCont *k) {
    ...
    /* Materialise the [Int] payload into a byte buffer. A datagram
     * over the 64 KiB IPv4 ceiling is out of scope for v1; sendto
     * will reject it with EMSGSIZE and we surface that. */
    size_t n = 0;
    for (KaiValue *p = data; p && p->tag == KAI_CONS; p = p->as.cons.tail) ++n;
    unsigned char *buf = NULL;
    if (n > 0) {
...
/* recv(s, max) -> Result[Pair[SocketAddr, [Int]], String]. Reads one
 * datagram of at most `max` bytes via recvfrom; the source address
 * is reformatted into a SocketAddr `{ host, port }` and returned as
 * the first half of the pair, the bytes as the second. max <= 0
 * panics per spec (no useful "read zero bytes"). A zero-length
 * datagram is legal and surfaces as `Ok(Pair { fst: src, snd: [] })`
 * — distinct from the TCP peer-close empty list because UDP has no
 * connection to close. */
static KaiValue *kai_default_netudp_recv(void *self, KaiValue *sock, KaiValue *max, KaiCont *k) {
    ...
    /* Format the source address back into a textual SocketAddr. */
    ...
    /* Build the [Int] payload tail-first so the head is byte 0. */
...
/* close(s) -> Unit. Errors from close(2) are logged and swallowed,
 * matching the NetTcp.close convention — the user can do nothing
 * useful with the value. Returns kai_unit(). */
 ...
 * code:
 *   - #440 Phase 2: raw scalar in `.i64`, slot_mask kind-1.
 *   - #741: re-boxed to a tagged pointer (`{.ptr = kai_int(n)}`,
 *     slot_mask 0) when stage 1's emitter bound every slot via `.ptr`.
 *   - i64-inline Lane B (#741 / commit 1f4f66f): back to raw int64 in
 *     `.i64` with slot_mask kind-1. `variant_slot_kind(Int) == 1`, so
 *     the emitter constructs `Exited` as
 ...
 * Mirror the emitter exactly: raw `.i64`, mask 1. mask 1 also keeps
 * the drop walker from decref-ing a raw int as if it were a pointer. */
 ...
 * receive uses this to disarm its deadline timer once a message wins. */
static int kai_reactor_timer_remove(KaiFiber *f) {
 ...
 * op performed in a fiber whose own body installed no handler for it has no
 * disposition to bind. Report it instead of dereferencing the NULL node. */
static KaiEvidence *kai_evidence_require(KaiEvidence *node, const char *eff_label) {
     ...
     * filled (a default global minted for a fiber-local effect with no real
