...
static KaiValue *_kai_process_make_child_fds(int pid, int in_fd, int out_fd) {
    KaiValue *fields[3] = {
        kai_int((int64_t) pid),
        kai_int((int64_t) in_fd),
        kai_int((int64_t) out_fd),
    };
...
static KaiValue *kai_default_process_start_piped(void *self, KaiValue *cmd, KaiValue *args, KaiCont *k) {
    ...
    close(out_pipe[1]);
    _kai_process_free_argv(argv, argc);
    KaiValue *child = _kai_process_make_child_fds((int) pid, in_pipe[1], out_pipe[0]);
    return kai_cont_resume(k, _kai_process_ok(child));
}
...
static KaiValue *kai_default_process_write_stdin(void *self, KaiValue *child, KaiValue *s, KaiCont *k) {
    (void) self;
    int fd = _kai_process_record_field(child, "stdin_fd");
    if (fd < 0) {
    ...
    _kai_process_ignore_sigpipe();
    const char *bytes = s->as.s.bytes ? s->as.s.bytes : "";
    size_t want = (size_t) s->as.s.len;
    size_t sent = 0;
    while (sent < want) {
...
static KaiValue *kai_default_process_read_stdout(void *self, KaiValue *child, KaiValue *n, KaiCont *k) {
    (void) self;
    int fd = _kai_process_record_field(child, "stdout_fd");
    if (fd < 0) {
        return kai_cont_resume(k, _kai_process_err_msg("read_stdout: child has no piped stdout"));
    }
    int64_t want = (n && kai_is_int(n)) ? kai_intf(n) : 0;
    if (want <= 0) {
    ...
    KaiValue *str = kai_str_from_bytes(buf, (size_t) got);
    free(buf);
