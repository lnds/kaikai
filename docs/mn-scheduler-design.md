...
**The one exception to "nothing is pinned": the entry fiber.** `main` is pinned to thread 0, the OS thread the process entered on, and is neither stealable nor re-homeable. This is not a locality optimization — it is a correctness requirement imposed from outside the runtime. A thread-affine C library traps rather than degrades when called off the process main thread: AppKit (and through it GLFW, raylib, SDL, GTK) checks and `brk`s, so before the pin every macOS GUI/game program died at window creation the moment the parallel scheduler became the default. Go pins for the same reason and calls it `runtime.LockOSThread`. The cost is bounded and paid by one fiber: `main` waits for thread 0 instead of running on the first free worker, and every fiber it spawns keeps migrating normally. Enforced at two points — the thief skips a pinned head, and `kai_sched_enqueue` routes a pinned fiber to slot 0 regardless of which thread the enqueue runs on.
...
- **Wakeup mechanism = destination reactor's self-pipe / eventfd.** When thread A pushes to a mailbox whose owner is PARKED on thread B's reactor, A writes one byte to B's wakeup pipe. This is *exactly* the `kai_filepool_pipe` pattern already in the tree — the file-pool worker writes a byte to wake the scheduler's `poll()`. We generalize: every scheduler thread has its own wakeup pipe in its own `poll()` set. Futex-on-Linux / condvar would work too, but the self-pipe unifies with the reactor's existing `poll()` loop (one wait primitive, not two) and is already portable across macOS/Linux. **Decision: per-thread wakeup pipe folded into that thread's poll set.** (An `eventfd` on Linux is a strictly-better self-pipe; use it under `#ifdef __linux__` as a perf refinement, self-pipe as the portable floor.)
  ...
  **As built:** F2 moved `poll()` onto the dedicated reactor thread (§4), so a scheduler thread has no poll set to fold a pipe into. A worker that runs out of work first searches for a bounded spell (`kai_worker_spin`), then parks on a per-slot mutex + condvar and is woken by a permit (`kai_worker_park` / `kai_sched_notify_work` in `stage2/runtime.h`). A producer pushes under the slot lock and then wakes the slot's owner if it is parked, else leaves the fiber to a spinning worker, else wakes one parked peer that can steal it. A worker announces itself parked *before* its final recheck of every deque, so a push is either seen by that recheck or finds the worker parked — no wakeup is lost. Parked workers never retry, so the global-quiescence deadlock check runs when a worker parks and when the reactor goes quiescent.
...
- **Region values (§1) never span a yield** (the pinned region invariant), so a migrating fiber carries no live arena pointer — the arena TLS is empty at every suspension point. This is why arena-stack-as-TLS is sound under migration. **Enforced, not just argued:** the pinned-region gate (`compiler/region_yield.kai`) fails the compile if any `region { }` body can reach a suspending effect (`Spawn`/`Actor`/`Cancel`/`Link`/`Monitor`) — directly or through a call, since the typed effect row already carries the transitive reach a syntactic scan would miss. So no region push can span a park; the arena `exposed` entries in `tools/tls-hoist.allow` rest on a checked property.
