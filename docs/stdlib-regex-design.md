...
8. **Docs**: update `stdlib-layout.md` to mark `regexp` as landed; mark m12.6.x #7 as unblocked (the refinement-side `where matches ~r/.../` sigil + predicate landed in a follow-up lane closing issue #85, 2026-05-03).
...
**Zero changes** to `stage2/compiler.kai` — pure stdlib lane. The `where matches ~r/.../` sigil syntax needs lexer/parser integration but that lands as **m12.6.x #7 lane**, separate from this one. Lane B unblocks it.
