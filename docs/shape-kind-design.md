...
**Status: SHIPPED.** `kind Shape : Structural`, the `Shape-App` formation rule,
...
fast-path) are all live. The `Functorial` theory is in the catalog, a protocol
declares it with a `laws Functorial` clause, and per-impl property checks are
...
**Decision — the law is declared by a closed theory *name*, NOT by inline
equations.**

> **Shipped as (2026-07-13):** a trailing `laws Functorial` clause after the
> protocol body (`protocol Sequence[s: Shape] { … } laws Functorial`), and the
> mirror `impl … { … } axiom Functorial` for the exemption. This keeps the
> decision's load-bearing half — the surface names a *closed theory*, never
