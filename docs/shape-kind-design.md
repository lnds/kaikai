...
fast-path) are all live. The `Functorial` theory is in the catalog. The atomicity
guard (arity-1, no composition) rejects at formation with clear diagnostics.

> **What is shipped as the sound floor, and what the theory buys next.** The generic
> fusion rewrite (`map`-into-`foldl` → a single `foldl` with the stage folded into the
> combiner) fires under the existing structural **purity gate**, exactly as `list`'s
> fusion does — it is sound for any `foldl` that is a genuine left traversal. The
> `Functorial` law's autogenerated property checks (§6) are the verification that a
> user's `foldl` *is* functorial; until an impl carries them, the rewrite trusts the
> author (as it must for the stdlib's own `list`). Opening the rewrite to arbitrary
> user impls with machine-checked soundness is the law-check follow-up, not a change to
