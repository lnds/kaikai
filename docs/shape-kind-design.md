...
**Status: SHIPPED.** `kind Shape : ConstructorApp`, the `Shape-App` formation rule,
`[s: Shape]` tparams, first-order applicative unification, static `O(1)` dispatch,
and generalised pipe fusion (a `Sequence[s: Shape]` container's `map`-into-`foldl`
pipeline fuses to a single traversal, `list`/`Vec` unchanged on their hand-written
fast-path) are all live. `Functorial` is a protocol-law set (not a theory — it decides no
type equality), a protocol declares it with a `: Functorial` header, and per-impl property checks are
autogenerated (`axiom Functorial` exempts an impl). The atomicity guard (arity-1,
no composition) rejects at formation with clear diagnostics.

> **The sound floor and its verification.** The generic fusion rewrite
> (`map`-into-`foldl` → a single `foldl` with the stage folded into the combiner)
> fires under the existing structural **purity gate**, exactly as `list`'s fusion
> does — it is sound for any `foldl` that is a genuine left traversal. The
> `Functorial` law's autogenerated property checks (§6) verify that a user's `foldl`
> *is* functorial: they run under `kai check` / `kai test` over witnesses enumerated
> from the impl type's constructors, comparing each law's two sides by scalar
> reduction through the impl's own `foldl`. An impl asserts `axiom Functorial` to opt
> out (as the stdlib's own trusted `list` does implicitly).

**Original status line (kept for the record):** proposal with a taken position; the
Tier 1 §3 amendment (§7) is an edition-level decision.

**Packaging (resolved):** `Shape` is a **catalog kind** — `kind Shape : ConstructorApp`,
...
HKT. A new kind `Shape : ConstructorApp` of atomic arity-1 constructors makes `s[A] ~
List[Int]` first-order applicative unification (decidable by construction — the same
way `Composition` already rejects `be*le` "at formation"); a new law set `Functorial`
...
fusion is decoupled from `stdlib/list`: **closed law set, open models.**
...
> error, walked back here. `Shape` is a **catalog kind** — `kind Shape : ConstructorApp`
...
### 1.1 `Shape` is a kind over `ConstructorApp`
...
pub kind Shape : ConstructorApp
```

**The theory is `ConstructorApp` — Shape's own engine.** Habitant equality is symbol
identity: `List ~ List` succeeds, `List ~ Vec` fails. No product, no inverse, no sum —
a shape "stands only for itself". The *application* rule (§1.3) is what makes this an
engine rather than plain equality: it decomposes `s[A]` into head and argument and
binds the constructor witness.

> An earlier draft shared one `Structural` label between `Shape` and `Region` on the
> grounds that habitant equality was "the same symbol identity, zero new engine". The
> engine audit rejected that: witness-binding and region branding are two mechanisms,
> and a theory names one engine. `Shape` now carries `ConstructorApp`, `Region`
> carries `Nominal`.
...
> and nobody extends them, because `HindleyMilner`/`EffectRow`/`ConstructorApp` are
...
```kaikai
# `Shape` classifies arity-1 type constructors. `ConstructorApp` gives identity:
...
Habitant *equality* is symbol identity (§1.1). What the assembled theories cannot give
is **application**: the property menu speaks only of combination *within* one kind
(abelian product of `Measure`, summed measure of `Composition`), never of applying a
habitant of one kind to a habitant of another. The application `Shape × Type → Type`
is a **formation rule in the typer**, and the witness-binding that decides it is what
makes `ConstructorApp` an engine of its own rather than a relabelled identity check.
...
**Decision — the law is declared with the theory in the header (`: Functorial`),
NOT as inline equations.** Shipped as written: the protocol header carries
`: Functorial` (reusing the `kind K : Theory` spelling), and the impl asserts
`axiom Functorial` after its body for the exemption.
...
|---|---|---|
| **New kind + application rule in the typer** | The `Kind` enum is today `KType \| KUnit` (2 variants); adding `KShape` is mechanical (precedent #1108: ~29 sites, 3 files). Habitant equality reuses `ConstructorApp` (zero engine). The one real new mechanism is the `Shape-App` formation/unification rule — pure first-order, cheaper than the abelian engine (no linear-equation solving). | Low. `ConstructorApp` is free; the application rule is the cheapest new formation in the family (less than `Composition`, far less than `AbelianGroup`). |
...
The *sound core* of this proposal (kind `Shape : ConstructorApp`, the `Shape-App`
...
**I would go for: amend Tier 1 and ship the `Shape : ConstructorApp` kind + `Shape-App`
