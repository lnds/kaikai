...
`Sequence` is post-1.0, exactly as GATs were post-1.0 in Rust — and when it lands it
...
**Orongo (1.0) or post-1.0? — Post-1.0, with one exception.**
...
1. **First, resolve the "list default" fork** (independent). This proposal defuses it:
   ...
   forces it.
2. **In Orongo (1.0): the surface lock, NOT the engine.** Reserve the `[s: Shape]` and
...
fusion as the first post-1.0 lane gated by the native bench — and resolve the "list
