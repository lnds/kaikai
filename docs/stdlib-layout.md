...
stdlib/
  ...
  os/            effects: Env, Process
    ...
    process.kai  (shipped — closes #346 + #1591; `start`, `wait`, `kill`, `exit` plus the pipe family `start_piped`, `pipe_to`, `pipe_from`, `pipe_errs`, `write_stdin`, `close_stdin`, `read_stdout`, `read_stderr`, `read_all`)
...
- `core.string` — length, slice, to_int, concat, starts_with, ends_with, trim, repeat, join *(originals)*; split, replace, pad_left, pad_right, lines, is_blank *(shipped via #338)*; **index_of, to_upper, to_lower, is_empty, reverse** *(shipped via #396 — the remainder of #338's proposed surface)*; drop_prefix, drop_suffix *(shipped via #632)*; **bytes, chars, char_count, byte_length, char_indices + `CharIndex` record** *(shipped via #744 — the codepoint model)*; **trim_left, trim_right, from_chars** *(shipped — closes #1589: one-sided trim + the `chars` inverse)*; **from_bytes + `bytes` retyped to `[Byte]`** *(shipped — closes #1978: the byte view wears the byte type)*; **`slice` re-based on codepoints + byte_slice, is_char_boundary, floor_char_boundary, ceil_char_boundary, char_index_of** *(shipped — closes #1982: the index unit is chosen, not implicit)*. `length`/`byte_length` count bytes; `char_count` counts Unicode codepoints; `bytes()` is the byte-level `[Byte]`, `chars()` the codepoint-level `[Char]` (UTF-8 decode) — distinct element types, so `from_chars(bytes(s))` is a type error instead of a silent double-encode; `from_bytes` is the `bytes` inverse, `from_chars` the `chars` one. Two index units, split by name: `slice` / `char_index_of` / `char_count` address codepoints; `char_at` / `index_of` / `length` / `byte_length` and the `byte_`-prefixed names address bytes. `slice(s, from, len)` takes codepoint indices and clamps out of range, so it can never return ill-formed UTF-8 (it used to index bytes and split a codepoint silently). `byte_slice(s, from, len) : Option[String]` is the byte-indexed cut, `None` when either end falls inside a multibyte sequence or outside the buffer — `Option` rather than Rust's panic, matching `to_int` / `drop_prefix` / `drop_suffix`. `is_char_boundary` / `floor_char_boundary` / `ceil_char_boundary` classify and snap an arbitrary byte offset (Rust 1.91's truncate-to-N-bytes primitives). The search pair stays coherent by shipping both halves: `index_of` keeps its byte offset and pairs with `byte_slice`; `char_index_of` returns a codepoint index and pairs with `slice`. `reverse`/`to_upper`/`to_lower` operate by codepoint. `split(s, "")` panics; `lines("")` returns `[]` (Python/Rust convention). `drop_prefix` / `drop_suffix` return `Option[String]` (`Some(rest)` on match, `None` otherwise) — same shape as `to_int`, `log2`, `div_mod`, mirroring Rust's `str::strip_prefix`. The #396 five share bare names with core exports (`list.is_empty`, `char.to_upper`, …); first-arg-type narrowing (#235) routes the `String` receiver. (A bare call inline in a comparison still mis-narrows cross-module — issue #758 — so the intrinsic tests bind with `let` before asserting.)
- `core.option` — is_some, is_none, map, and_then, unwrap_or, or_else
- `core.result` — is_ok, is_err, map, map_err, and_then, unwrap_or
- `core.char` — is_digit, is_lower, is_upper, is_alpha, is_alnum, is_space, to_lower, to_upper. ASCII-only classification and case mapping.
- `core.char_unicode` — is_lower, is_upper, to_lower, to_upper. Simple, locale-independent case mappings for ASCII, Latin U+00C0–U+017F, basic Greek U+0391–U+03A9 / U+03B1–U+03C9, and Cyrillic U+0400–U+045F. Latin Extended-A includes the alternating pair ranges and exceptional mappings (`İ` → `i`, `ı` → `I`, `ſ` → `S`, `Ÿ` ↔ `ÿ`). Multi-character mappings and contextual casing are excluded; `ß` and `ŉ` stay unchanged on uppercase. `string.to_lower_unicode` / `string.to_upper_unicode` apply these mappings to strings.
...
- `os.process` — `start(cmd, args)`, `wait`, `kill`, `exit` *(shipped — closes #346)*, plus the popen-shaped pipe family `start_piped`, `pipe_to`, `pipe_from`, `write_stdin`, `close_stdin`, `read_stdout`, `read_all` *(shipped — closes #1591; wraps the `start_piped`/`write_stdin`/`close_stdin`/`read_stdout` ops of `Process`)* and stderr capture `pipe_errs`, `read_stderr` *(shipped — closes #2089; `start_piped` takes a `pipe_stderr` flag and `read_stderr` is its op. `wait_or_kill` and `signal` still deferred — `wait_or_kill` is unblocked (R1 reactor shipped 2026-05-15, #611, parks the fiber on `Process.wait` via SIGCHLD self-pipe) and waits on the cancel-aware wrapper, see `docs/effects-stdlib.md` §`Process` v1-status sidebar.)*
...
- `#[derive(Json)]` — typed struct binding over the same DOM *(shipped: `to_json` / `<lower(T)>_of_json` for records and sum types; field names verbatim, `Option` accepts null-or-missing, unknown keys ignored, failures are `Result[T, JsonError]` carrying the JSON path. Sum types encode under a fixed adjacently-tagged envelope `{"tag", "value"}` (nullary → `null`, single → the value, multi → a JSON array); decode routes on `tag`, an unknown tag is a located error. Per-field `#[json(rename/default/skip)]` overrides layer on top: `rename` sets the wire name, `default` supplies an absent key's value, `skip` drops a field (requiring a `default` or `Option`). `protocol Json` in `stdlib/protocols.kai`; `JsonValue` / `JsonError` / runtime in `stdlib/encoding/json_bind.kai`, kept out of the core so `JNull` does not offset every binary's RC baseline. See `docs/json-derive-design.md`.)*
- `encoding.toml` — `decode`, `decode_report`, `encode`, `round_trip`, `error_message` *(shipped subset for the package manager (#405): top-level scalars, `[name]` and `[[name]]` headers, basic strings, ints/bools, array values (#1742, the `[native]` table), inline tables, comments. Floats via #2056 — `TReal(Real)`, optional `-`/`+` sign, fractional (`0.8`) and/or exponent (`5e+22`) parts, a digit required on each side of the `.` and after the exponent marker; the encoder appends `.0` so an integral float does not re-decode as an integer. `inf` / `nan` and `_` digit separators stay out of the subset. `decode_report` (#2056) returns `Result[TomlValue, TomlError]` naming the 1-based line and bare key the parse stopped on, so a caller can point at the offending entry instead of calling the whole file malformed; `error_message` renders it. Encoder emits a canonical form so `kai.lock` round-trips byte-identical. Datetimes / dotted keys / multi-line strings deferred until a user-facing TOML need lands.)*
