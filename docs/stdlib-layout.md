...
- `collections.set` — list-backed insertion-ordered set: empty, insert, remove, contains, union, intersect, diff, size, is_empty, to_list, from_list; `map`, `flat_map`, `filter` exported with element shape `a` so `Set[a]` participates in `|`, `||`, `|?` (shipped via #876). `map` collapses output collisions.
