...
stdlib/
  ...
  os/            effects: Env, Process
    ...
    process.kai  (shipped — closes #346 + #1591; 9 public fns: `start`, `start_piped`, `write_stdin`, `read_stdout`, `read_all_stdout`, `close_stdin`, `wait`, `kill`, `exit`)
...
- `os.process` — `start(cmd, args)`, `wait`, `kill`, `exit` *(shipped — closes #346)*, plus `start_piped`, `write_stdin`, `read_stdout`, `read_all_stdout`, `close_stdin` *(shipped — closes #1591; pipes on the child's stdin/stdout, stderr still inherits. See `docs/effects-stdlib.md` §`Process` *Stdio redirection* for the bounded-read and SIGPIPE contracts.)*. `wait_or_kill` and `signal` still deferred — `wait_or_kill` is unblocked (R1 reactor shipped 2026-05-15, #611, parks the fiber on `Process.wait` via SIGCHLD self-pipe) and waits on the cancel-aware wrapper, see `docs/effects-stdlib.md` §`Process` v1-status sidebar.
