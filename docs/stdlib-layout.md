...
- `math.vector` *(shipped)* — `Vec2` / `Vec3` vector math types and pure operations for games and graphics: constructors, constants (zero, up, down, etc.), arithmetic (add, sub, scale, neg), length/normalize, distance, dot/cross, lerp, angle, reflect — all prefixed `vec2_` / `vec3_` for selective import and unqualified use *(closes the doc-gap where `docs/kaikai-minimal.md` and `docs/stage2-design.md` reference `math.vector` as an import example without the file existing)*.
