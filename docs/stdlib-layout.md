...
- `encoding.toml` — `decode`, `encode`, `round_trip` *(shipped subset for the package manager (#405): top-level scalars, `[name]` and `[[name]]` headers, basic strings, ints/bools, array values (#1742, the `[native]` table), inline tables, comments. Encoder emits a canonical form so `kai.lock` round-trips byte-identical. Floats / datetimes / dotted keys / multi-line strings deferred until a user-facing TOML need lands.)*
