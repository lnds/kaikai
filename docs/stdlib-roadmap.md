...
collections/  map.kai hashmap.kai set.kai hashset.kai stack.kai queue.kai vec.kai
math/  int.kai real.kai numeric.kai bits.kai complex.kai
crypto/  hash.kai mac.kai
encoding/  json.kai json_bind.kai toml.kai base64.kai hex.kai
...
| Module                                  | Status                                                                                                                         |
|-----------------------------------------|--------------------------------------------------------------------------------------------------------------------------------|
| `fs/file`                               | shipped via PR #132 (3 public fns: `file_read_file`, `file_write_file`, `file_append`); minimum-viable extras shipped via #345 (`file_exists`, `file_delete`, `file_rename` as core builtins riding `File`); module-qualified surface `file.exists` / `file.delete` / `file.rename` shipped via #423 (closes the gap where `import fs.file` did not expose the #345 ops); `metadata`/`read_bytes`/`write_bytes` deferred (need record type / `[Int]` byte-array conversion); chunked streaming ops `open_read`/`read_chunk`/`open_write`/`write_chunk`/`close_file` + `FileHandle` shipped via PR #804 (issue #771 Phase 1, R1-pool default handlers); lazy line surface shipped via issue #801 (`stdlib/stream.kai` — push carrier, pipe-canonical combinators, `read_lines`/`write_lines`, `ReadFault` recovery; supersedes #771 Phase 2 bracket surface); typed capabilities on `FileHandle` shipped via PR #1452 (closes #1251) (`Perm`/`Semilattice`: `open_read` → `FileHandle<read>`, `open_write` → `FileHandle<read + write>` over an `O_RDWR` fd, per-op capability requirements, `file_handle(fd)` minting for handlers/tests) |
...
| `os/process` (runtime + effect)         | shipped via PR #142 (closes the runtime side of #126); popen-shaped stdio pipes (`start_piped`/`write_stdin`/`close_stdin`/`read_stdout` ops, pclose-shaped `wait`) shipped via #1591; `pipe_stderr` flag + `read_stderr` op shipped via #2089 |
| `os/process` (Kai wrapper)              | shipped (closes #346): `process.start`, `process.wait`, `process.kill`, `process.exit`; pipe surface (`start_piped`, `pipe_to`, `pipe_from`, `write_stdin`, `close_stdin`, `read_stdout`, `read_all`) shipped via #1591; stderr capture (`pipe_errs`, `read_stderr`) shipped via #2089 |
...
| `#[derive(Json)]` typed struct binding  | shipped via PR #1489 (closes #1280): `#[derive(Json)]` emits `impl Json for T` (`to_json` / `of_json`) plus a `<lower(T)>_of_json` shim, over records of `String`/`Int`/`Real`/`Bool`/`[T]`/`Option[T]`/nested-record fields. Four binding policies fixed in the derive: field names go to the wire verbatim (no case conversion); `Option[T]` accepts an explicit `null` and a missing key alike, every other field is required; failures are `Result[T, JsonError]` carrying the JSON path to the offending node (`address.boxes[2].zip`); unknown keys are ignored. `protocol Json` in `stdlib/protocols.kai`; `JsonValue` / `JsonError` / runtime in `stdlib/encoding/json_bind.kai` (kept out of the auto-loaded core so `JNull` is not eager-seeded into every binary); impl builder in `stage2/compiler/json_derive.kai`. Sum types derive under a fixed adjacently-tagged envelope `{"tag": "<Variant>", "value": <payload>}` (closes #1488): nullary → `value: null`, single-payload → the lone encoding, multi-payload → a JSON array; decode routes on `tag` and decodes `value` against the variant's payload, an unknown tag is a located `JsonError` naming the accepted variants. `Option[Option[T]]` and generic payloads/fields are rejected at the derive with a located diagnostic. Per-field `#[json(rename = "wire")]` / `#[json(default = <expr>)]` / `#[json(skip)]` overrides layer on top (closes #1487): `rename` sets the wire name, `default` supplies a value for an absent key without weakening the required-field policy, `skip` drops a field from the wire and requires a `default` or `Option` to reconstruct it on decode; a `#[jsom]` typo is a compile error. Fixtures `examples/stdlib/json_derive_{roundtrip,sum_roundtrip,absence,error_path,overrides,jsom_typo,skip_no_default,multi_combinator}.kai` + `examples/negative/derive/derive_json_{sum_type,unbindable_field}.kai` + `examples/multi-module/issue-{1852-cross-module-derive-json,1870-derive-json-contested-field}/` |
| `encoding/toml` floats + located decode errors | shipped via PR #2068 (closes #2056): `TReal(Real)` joins `TomlValue`; the decoder reads signed floats with a fractional part, an exponent, or both (`0.8`, `-0.01`, `+2.5`, `1e6`, `5e+22`, `6.626e-34`), requiring a digit on each side of the `.` and after the exponent marker — `inf`, `nan` and `_` digit separators stay out of the subset. The encoder appends `.0` when `real_to_string` prints an integral value with no `.` or exponent, so a float round-trips as a float. `decode_report` returns `Result[TomlValue, TomlError]` with the 1-based line and bare key the parse stopped on (`error_message` renders `line 5: key threshold: unsupported or malformed value`); `decode` keeps its `Option` shape. Fixtures `examples/stdlib/toml_{float_values,decode_report}.kai` |
...
| `core/string` one-sided trim + `from_chars` | shipped via PR #1629 (closes #1589): `string.trim_left` / `string.trim_right` remove whitespace (same set as `trim`) from one end only, reusing the `trim` scan loops — the primitive indentation-significant formats need; `string.from_chars : [Char] -> String` UTF-8 encodes each codepoint, the inverse of `chars` (encoder built on `int_to_byte_string`, no new runtime prim). Same lane fixed `to_upper`/`to_lower` multibyte corruption — the char rebuild went through `Show[Char]` interpolation, emitting raw low bytes for non-ASCII. Fixtures `examples/string/{trim_sided,from_chars_roundtrip,case_fold_multibyte}.kai` |
| `core/string` byte type + `from_bytes` | shipped via PR #1979 (closes #1978): `string.bytes(s)` returns `[Byte]` instead of `[Char]`, so the byte view no longer wears the codepoint type — `from_chars(bytes(s))` was well-typed and silently double-encoded every non-ASCII byte (`café` → `cafÃ©`). Reads octets through `string_byte_at_int`, no new runtime prim. `string.from_bytes : [Byte] -> String` is the inverse of `bytes`, as `from_chars` is of `chars`; the two round-trips no longer typecheck across each other. **Breaking** — callers consuming the elements as `Char` convert with `byte_to_int`. The huffman demo moved its symbol alphabet to `Byte` and dropped its ASCII lookup table, gaining correct non-ASCII round-trips. `http`/`random_secure` keep `[Int]` deliberately. Fixtures `examples/string/from_bytes_roundtrip.kai` + `examples/negative/type_invariants/from_chars_rejects_bytes.kai` |
| `core/string` index unit + boundary primitives | shipped via PR #1985 (closes #1982): `string.slice(s, from, len)` now indexes **Unicode codepoints** instead of bytes — it used to cut a multibyte sequence in half and return ill-formed UTF-8 with no error (`slice("café", 2, 2)` gave `f\xef\xbf\xbd`). Out-of-range indices clamp. `string.byte_slice(s, from, len) : Option[String]` keeps the byte-indexed cut and returns `None` when either end falls inside a codepoint or outside the buffer — `Option` rather than Rust's panic, consistent with `to_int` / `drop_prefix` / `drop_suffix`, none of which panics on bad input. `is_char_boundary` / `floor_char_boundary` / `ceil_char_boundary` are the truncate-to-N-bytes-without-splitting primitives Rust stabilised in 1.91. Coherence of the search/slice pair: rather than move `index_of` off byte offsets, both halves ship — `index_of` (bytes) pairs with `byte_slice`, `char_index_of` (codepoints) pairs with `slice`; the byte offset stays available for `char_indices` and the boundary primitives, and neither pair can feed the other's index unit by accident. **Breaking** — a caller slicing ASCII is unaffected (one codepoint is one byte); a caller feeding `index_of` into `slice` moves to `byte_slice` or `char_index_of`. Migrated: `stdlib/stream.kai` (cut at a `\n` offset) and `demos/9d9l/{weather,vectores}` (byte-level by construction). Fixtures `examples/string/{slice_codepoints,byte_slice_boundary}.kai` |
| `array` bridge module                   | shipped (closes #366): top-level `stdlib/array.kai` with `array.from`, `array.to_list`, `array.copy` (sentinel-free since #1015 L1 — `from`/`copy` seed from the input, empty routes through the `array_empty` prim). `random.shuffle` flipped from O(n²) selection-sampling to O(n) in-place Fisher-Yates over a locally-built `Array[T]`; observable row stays `[T] -> [T] / Random` (masking pass drops the inner `Mutable`) |
| `decimal` div-by-zero hardening         | shipped (closes #363): `dec_div` returns `Option[Decimal]` (`None` on zero divisor) instead of silent `dec_zero`. **Breaking** — direct callers must `match` the result. |
...
| `net/http` automatic redirect following  | shipped (closes #357): `stdlib/net/http.kai` gains `RedirectPolicy` (4 fields: `max_redirects`, `follow_3xx`, `rewrite_post_to_get_on_303`, `preserve_method_307_308`), `default_redirect_policy`, `http_follow`, plus 4 convenience wrappers (`http_get_follow`, `http_post_follow`, `http_put_follow`, `http_delete_follow`). RFC 9110 §15.4 method-rewrite rules baked in; bare `http_request` and the existing `http_get`/`http_post`/`http_put`/`http_delete` keep their single-call semantics (no breaking change). Cross-origin redirects strip Authorization, Proxy-Authorization, Cookie and Cookie2 case-insensitively; same-origin hops preserve them, and stripped credentials never return on later hops. Host overrides are removed on every hop. Cookie persistence remains deferred. Selfhost byte-identical. |
...
| TTY detection (`effects.kai`)            | shipped (closes #1590): `is_tty() : Bool` op on `Stdout` / `Stderr` / `Stdin`, bridging to `isatty(3)` on fd 1/2/0 via default handlers `kai_default_{stdout,stderr,stdin}_is_tty` (stage2/runtime.h + LLVM forwarders in runtime_llvm.c). The ANSI-colour gate; pairs with the `NO_COLOR` user override already served by `Env.get`. Fixture `examples/effects/isatty_pipe_false.kai` (false under pipe on all three fds, both backends); the true half runs under a pseudo-TTY in `test-issue-1590-isatty` where util-linux `script` is available (Linux CI) |
...
| `decimal_big` + `rational` (top-level)   | shipped (closes #514 — numeric lane D): `stdlib/decimal_big.kai` is `Decimal` with a `BigInt` carrier (`{ raw: BigInt, scale: Int }`) — no ~38-digit Int128 ceiling, `add`/`sub`/`mul` total, `div` at an explicit truncating target scale. `stdlib/rational.kai` is exact `{ num: BigInt, den: BigInt }` kept canonical (positive denominator, sign in numerator, reduced by Euclidean `gcd` at every construction); `add`/`sub`/`mul`/`div`/`recip` stay reduced. `Show`/`Eq`/`Ord` for both (`*_proto`). Reuses lane B's BigInt API; pure stdlib, no runtime or compiler change. Fixtures `examples/numeric/{decimal_big_demo,rational_demo}.kai` (C + native parity). Closes the #514 numeric chain (A: fixed-width primitives, B: BigInt, C: Decimal-Int128, D: this) |
