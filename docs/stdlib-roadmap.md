...
collections/  map.kai hashmap.kai set.kai stack.kai queue.kai
...
os/  args.kai env.kai
top-level: actor.kai array.kai decimal.kai effects.kai fx.kai log.kai
           loop.kai money.kai path.kai protocols.kai random.kai
...
| `collections/hashmap` HAMT hash map | shipped (closes #374, unblocks #375 HashSet): `stdlib/collections/hashmap.kai` — Hash Array Mapped Trie carrier (`HEmpty`/`HLeaf`/`HCollision`/`HBranch`), `HashMap[k, v] = { root, size }`. Branching factor 32 (5 bits/level), bitmap-compressed children (only `popcount(bitmap)` slots allocated). 12 flat `hashmap_*` pub fns exactly as the issue pins (`hashmap_empty/size/is_empty/get/contains/put/remove/keys/values/to_pairs/from_pairs/merge`). Pure / persistent; iteration order hash-derived (unspecified) but stable across same-input runs. Keys need `impl Hash` + `==`; primitives and user **sum types** dispatch through the generic boundary, user **records** do not yet (pre-existing compiler dispatch gap, same class as the AVL `Map`'s `<` on non-primitives — documented as a follow-up). The `m[key]` read-side sugar does NOT dispatch to `hashmap_get` yet: that lives in the typer (`synth_index`), out of the stdlib lane's scope (one-arm follow-up). No HashDoS mitigation in v1. Fixtures `examples/stdlib/hashmap_basic.kai` (primitive-key API, 10K + 100K round-trip, iteration-order stability) and `examples/stdlib/hashmap_collision.kai` (forced `HCollision` via a constant-hash sum-type key). Benchmark in `benchmarks/hashmap/` — at these N the AVL `Map` is still ~1.6–2.3× faster (list-backed bitmap children + boxed-Int RC bury the asymptotic win); crossover deferred to an Array-backed-children + Int-unbox follow-up. Selfhost byte-identical (additive). |
