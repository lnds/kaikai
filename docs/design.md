...
- **Concrete syntax consolidation** (kaikai-minimal stabilised; resolved item-by-item):
  ...
  - collections `[]` / `()` / `{}` — **resolved**: `[]` for ordered lists, `{}` for records (named fields), `()` for grouping plus parser-only n-tuple sugar (cap N = 4). The "should `()` gain a tuples-as-a-type meaning" sub-decision was closed on 2026-04-27 with verdict **REJECT** (m8.5 gate; methodology and metrics in `docs/proposed-extensions.md` §9). Anonymous products use `Pair[a, b]` / `Triple[a, b, c]` / `Quad[a, b, c, d]` (stdlib); the n-tuple parser sugar `(a, b)` / `(a, b, c)` / `(a, b, c, d)` landed 2026-05-03 (PR #155) as syntactic shorthand over those records — no new `TyCtor`, no positional `.0` accessors, named accessors only (`.fst .snd .trd .frt`).
