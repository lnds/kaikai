...
| **Stranded sleeper on the ownerless `-O2` build** ⚠ open (#1348) | — | `examples/effects/mn_recv_timeout_wheel.kai` and `examples/effects/mn_fiber_free_race.kai` deadlock on a minority of runs (≈13% and ≈5% at `KAI_THREADS=8`) once the emitted C is compiled as one TU at `-O2` with no `-O0` scheduler owner — the build shape the `stage2` Makefile recipes use. Every worker idles in `kai_worker_loop`, the reactor blocks in `kai_reactor_wait`, nothing is runnable and no output is produced: a lost wakeup, not a crash. Invisible until the M:N gates gained that build arm; `bin/kai` builds are clean because the owner split suppresses it. |
