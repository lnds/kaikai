...
| ~~**Multi-threaded scheduler**~~ ✅ **shipped, on by default** | — | M:N work-stealing landed (design `docs/mn-scheduler-design.md`): state partition + isolated reactor, then the M:N scheduler, then the reactor serving N schedulers, then the default flip. `KAI_THREADS` unset resolves to the host CPU count (capped at 32); `KAI_THREADS=1` is the escape hatch and stays byte-identical. Cross-thread sends are physically copied so per-fiber RC stays non-atomic — the "requires atomics" concern was resolved by copy-at-the-boundary, not atomics. TSAN, ASAN, macOS M:N determinism and a corpus-wide N>1 determinism gate on both backends guard it. Measured against Elixir 1.20.2/OTP29 in `benchmarks/mn-throughput/`: ~3.6x serial (3.74x native, 3.72x C — the backends are within 1% of each other since #1354), 10.5x scaling on 16 cores. |
...
| ~~**Cross-generation unpark on the dual-park receive**~~ ✅ **closed** | — | A wake delivered to a LATER park of the same fiber. The reactor drains splice fibers off the wheel under `kai_reactor_mu` but unpark them after releasing it; in that gap a message send can wake the fiber off the recv chain, run it, and re-park it — the reactor's batched unpark then flips the NEW park READY. `receive_timeout` read that resume as its deadline firing: it returned a premature `None` while still linked into the wheel, and its next park re-inserted the same node, corrupting the wheel's list and the parked accounting until a sleeper stranded. The same stale-wake shape existed via `wake_pending` permits consumed at a later park, and `kai_sched_remote_unpark` could classify the target under a stale home slot's lock. Closed by making every park site treat a resume as a hint, not a proof: reactor drains stamp `reactor_fired` when they splice, sites re-check their own predicate (deadline elapsed, message present, child terminal) and re-park on a spurious resume, `kai_sched_park` detaches any reactor link left behind by a non-reactor wake, park commits consume pending wake permits on both the slot and reactor branches, and `remote_unpark` locks the target's current home slot via the retry loop and never writes permits into terminal fibers. Guarded by `examples/effects/mn_recv_timeout_deadline_race.kai` (deadline set equal to the producer's pacing so both wake sources collide on nearly every receive) on the M:N determinism gate. |
| ~~**Deadlock-detector false positive in the dispatch window**~~ ✅ **closed** | — | `kai_worker_loop` dequeued a fiber and only then left the idle pool, so a worker preempted between the dequeue and the `kai_sched_idle_count` decrement held a fiber that was invisible to the global-quiescence check: off every deque, not RUNNING, its worker still counted idle. With everything else asleep the check observed "all workers idle, deques empty, fibers parked", declared a terminal wedge, and killed a healthy process — the same `all workers idle with fibers parked` report a true strand produces, indistinguishable in CI. Closed by leaving the idle pool BEFORE searching for work, so a worker is non-idle from before it can acquire a fiber until the dispatch completes and the quiescence argument is airtight again. |
| ~~**Idle workers polled instead of blocking**~~ ✅ **closed** | — | An idle worker retried its deque search on a 200 µs `nanosleep`, so an M:N program burned CPU whenever it had fewer runnable fibers than workers: one that only sleeps used ~40% of a core at the default `N`. Idle workers now spin for a bounded spell, then park on a per-slot condvar; a producer wakes the slot owner or a parked peer with a permit unless a spinning worker will pick the fiber up. A worker announces itself parked before its final recheck of every deque, so no wakeup is lost; the deadlock check runs as a worker parks and as the reactor goes quiescent. Gate `test-mn-idle-cpu` (fixture `examples/effects/mn_idle_sleep_cpu.kai`) bounds the process CPU of an all-sleeping run. |
| ~~**Fiber switches invisible to ThreadSanitizer**~~ ✅ **closed** | — | Not a scheduler defect, but it made the M:N merge gate report on a runtime it could not follow. TSAN keys its shadow stack and sync clocks to the OS thread; a `swapcontext` is invisible to it, so a fiber pushed frame records on the thread that started it and popped them on the thread that resumed it. The imbalance is a random walk, and one bad walk ran a shadow stack off its mapping — a store fault inside the sanitizer that the process then spun on at 100% CPU with no race report and no crash, presenting as an intermittent deadlock (~1 run in 75 at `KAI_THREADS=4`, never without a sanitizer in 700 runs). Fixed by annotating every fiber switch with the TSAN fiber API. Two details are load-bearing and cost a round each to find: the switch is a macro, because a helper function changing fiber identity in its middle pushes its own entry record on the outgoing fiber and pops it from the incoming one, leaking one per switch; and each thread's root handle is captured eagerly at the top of its scheduler loop, because `__tsan_get_current_fiber()` called from inside a fiber aliases the root onto that fiber's state. The annotations compile away outside a TSAN build. Regression fixture `examples/effects/mn_deep_stack_migration.kai` — yield points deep in a non-tail call chain, so each steal migrates a tall stack of live frames — fails 40/40 unannotated at N=4 and is clean at N=1; on both the TSAN gate and the determinism gate. |
| ~~**`main` migrated off the process main thread**~~ ✅ **closed** | — | The default flip to the parallel scheduler put the entry fiber on thread 0's deque as an ordinary stealable fiber, so a worker picked it up and user `main` ran on a worker thread. A thread-affine C library traps rather than degrades when called from there: AppKit checks its caller and executes a bare `brk` inside `NSUpdateCycleInitialize`, so on macOS **every** GLFW/raylib/SDL program — the whole GUI/game class — compiled clean and died at window creation with SIGTRAP and no kaikai-level diagnostic. `KAI_THREADS=1` was the only workaround, which cost the user the entire parallel scheduler. Closed by pinning the entry fiber to thread 0 (the OS thread the process entered on) — the shape Go exposes as `runtime.LockOSThread` — enforced at the two points that can move a fiber: the thief skips a pinned head, and `kai_sched_enqueue` routes a pinned fiber to slot 0 no matter which thread the enqueue runs on. Only the entry fiber is pinned; everything it spawns migrates as before, so the load balance the lane bought is intact. Regression fixture `examples/ffi/main_thread_affinity.kai` asserts the property directly through a `pthread_main_np`/`gettid` shim (no GUI dependency in CI) and sweeps `KAI_THREADS` 1/2/4/8 on both backends — N=1 always passed, so a default-only fixture would have gone green on the broken runtime. |
...
dispatches through its clause exactly as a synchronous `Cancel.raise()`
...
`with Cancel` in the chain, so the supervisor observes the termination
through its mailbox. That is deliberate — OTP-style layered supervision
would not compose otherwise — and is specified in `docs/actors.md`
§*Trap-exit semantics* (issue #103).
