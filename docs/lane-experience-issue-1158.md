...
The redesigned lane shipped on the KIR spike base (branch
`spike-1158-kir`): inlining moved post-Perceus onto lowered KIR, where
RC ops are explicit statements and the TCO/TRMC spine is typed
terminators. Everything the first attempt fought by hand died by
construction, as the spike predicted — no RC bug class reappeared.

### Scope as planned vs as shipped

Planned: admit the `balance_left` reuse-token class, raise the
threshold, deliver the while-loop lever, hold the gates. Shipped, in
three coupled pieces:

1. **Reuse-class admission** (`kir_inline.kai` + `kir_inline_clone.kai`).
   `KConReuse`/`KRecordReuse`/`KTokenSteal`, the `KFreeToken`/
   `KIncrefIfShared` RC ops, and the `kaix_check_unique` /
   `kaix_incref_if_token_null` runtime helpers admit under a wider
   ceiling (128 stmts / 32 blocks vs 16 / 8 plain). The feared
   `_arm_ru` alpha-rename turned out NOT to exist as a problem: the
   arm-top token machinery lives in the CALLER (an arm whose body holds
   a TRMC step), never in a leaf template's body — `balance_left`'s own
   body carries only local token-steal / con-reuse machinery whose
   donors are template binders, all renameable. `KArmTopReuse` /
   `KArmTokenFree` stay inadmissible on principle (fn-wide register).
   Raw params materialise ONCE at the paste head (the call ABI's single
   boxed→raw border) — substituting boxed atoms into every use cost ~4%
   on rb-tree before this.

2. **Loop-template pastes + beta reduction** (`kir_inline_beta.kai`).
   `KTcrecGoto` templates paste as local loops (params become head
   registers; the back-edge re-creates the backend's eval → masked-drop
   → store sequence; identity re-assigns skip their store but keep the
   masked drop). Indirect calls then beta-reduce over single-assignment
   `KClosure` bindings, pasting lambda bodies with `PCapture` params
   bound to capture atoms and the consuming apply's ref retired in the
   join block. Dead closures (only dup/drop uses, alias-id captures)
   are swept.

3. **var-cell State→slot** (`kir_varslot.kai`). The issue comment's
   mechanism ("once the closures are gone the var→slot specialisation
   re-applies") does not hold as stated: var→slot decides in DESUGAR,
   long before KIR exists, so the fused loop still carried the canonical
   State handler. The lane re-derives the rewrite at KIR level:
   a canonical var-cell install (structural match on the desugar-minted
   get/set clause pair) whose alias id has no remaining atom reads and
   whose `@alias` performs are the exact get/set protocol becomes a
   mutable register. Soundness premise is the same one desugar's rewrite
   stands on: cells are addressable only through their alias id.

### Numbers (mac arm64, best-of-N interleaved)

- **while-loop lever (the language-wide win)**: LCG loop, N=50M,
  native, identical outputs. Dispatched State handler 1.98 s → fused +
...
- **C backend**: byte-identical emission (KIR feeds only native), the
  `--emit=kir` goldens moved to `--emit=kir-noinline` so they pin the
  raw lowering.

### Structural surprises

- **`_arm_ru` was a caller-side concern all along.** The issue text
  (and the spike's own flag) assumed the balance class needed a token
  alpha-rename design; reading the actual lowering showed
  `arm_is_top_reuse` requires a TRMC step in the arm body, which a leaf
  template can never have. The "design work" was admission bookkeeping.
- **The var→slot ordering premise was wrong** (see above) — the lane
  grew a third pass nobody scoped. It is the majority of the win.
- **A pattern binder named `args` miscompiles silently** under the
  stage1-built compiler: it shadows the builtin and binds empty. Cost
  one probe-build cycle to find; renamed to `vargs`. The beta pass
  "worked" before the fix only because loop lambdas take zero direct
  args.
...
- `examples/perceus/kir_inline_trmc_tail_1158.kai` +
  `test-kir-inline-trmc-1158`: leaf helper inlined ahead of a
  modulo-cons step; 1M build must stay O(1)-stack and the KIR must show
  the helper gone with `trmc-step` intact.
- `examples/perceus/kir_while_slot_1158.kai` +
  `test-kir-while-slot-1158`: while over two var cells; the KIR of
  `main` must carry `__vcell_` registers and no State install/perform.
- Both wired into `tier1-native.yml`.
