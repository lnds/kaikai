...
### Report modes over a package

`kai typecheck` and `kai lint` take the same specs `kai build` does
(nothing, `.`, `./<sub>`, or a file). Over a package with an entry
they run once from that entry, so the whole import graph is covered
in a single pass. Over a package with no entry there is no graph to
walk, so every module the package owns — plus the `tests/` sibling —
is checked as its own root, the way `kai fmt` formats them. A
directory with no manifest and no entry is still a usage error: the
package form widens what a package reaches, it does not make every
directory a target.

The exit codes are unchanged by this: `kai typecheck` exits 1 when
any module fails the front end, and `kai lint` keeps the exit code
it has in file mode — 0 over a module whose types do not check,
because a lint never blocks on a diagnosis it did not make.
...
| `library_report_modes/` | `kai typecheck` / `kai lint` over a package with no entry point: every module is reached, an ill-typed one fails typecheck, and lint still exits 0. |
