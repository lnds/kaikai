...
# Find all .err.expected goldens and pair them with their .kai sources.
...
find "$ROOT/examples/negative" -name '*.err.expected' -not -name '*.run.err.expected' -not -path '*/silent_contract/*' 2>/dev/null | sort | while read -r exp; do
  base=$(basename "$exp")
  dir=$(dirname "$exp")
  ...
  run_one "$src" "$exp" "$compiler" "$prelude" "$extra_flags"
done > "$tmp/results"
cat "$tmp/results"
